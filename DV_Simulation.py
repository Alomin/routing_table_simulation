...
parser = argparse.ArgumentParser(prog='DV vector test')
parser.add_argument('localport', help='local port number process listen to')
parser.add_argument('timeout',  help='intertransmission time of route update')
parser.add_argument('triples', nargs='*',help = 'all the triples')
args = parser.parse_args()

if not (args.localport.isdigit() and args.timeout.isdigit()):
    print "port num and timeout have to be digit num"
    sys.exit()
...
for i in range(0,len(args.triples),3):
    ...
    for a in test:
        if not (a.isdigit() and 0<=int(a)<256):
            print "addr input error"
            sys.exit()
    if not (args.triples[i+1].isdigit() and isfloat(args.triples[i+2])):
        print "addr input error"
        sys.exit()
    remoteaddr = (args.triples[i], int(args.triples[i+1]))
    weight = float(args.triples[i+2])
...
## update local DV: view all link and DV key, merge, new DV key, for every key, value = min (for l in link l + D(dest)(if exist))
def computeDV():
    ## if link change:
    dest = set(link.keys())
